(function (parser, reporter, getGitData, logger, Promise, util) {
    ...
    module.exports = function (input, opts) {
        ...
        loggerImpl.info(util.format('Handling input for: token [%j], accountToken [%j], username [%j], projectName [%j], commitId [%j], language [%j], partial [%j], final [%j], endpoint [%j], format [%j], path prefix [%j], verbose [%j], debug [%j]',
            token, accountToken, username, projectName, commit, language, partial, final, opts.endpoint, format, pathPrefix, opts.verbose, opts.debug));
        ...
        return Promise.all([parser.getParser(format).parse(pathPrefix, input), getGitData.getCommitId(commit)]).spread(function (parsedCoverage, commitId) {
            ...
            }).sendCoverage(token, commitId, language, partial, final, parsedCoverage, accountToken, username, projectName);
        });
    };
