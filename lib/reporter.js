(function(request, Joi, Promise, util, lodash, logger) {
    ...
    function sendByLanguage(endpoint, commitId, partial, final, token, data, accountToken, username, projectName) {
        ...
        var languageResponses = lodash.map(reportsByLanguage, function(fileReports, language) {
            ...
            return sendLanguage(endpoint, commitId, language, partial, final, token, dataPerLanguage, accountToken, username, projectName);
        });
    ...
    function sendForLanguage(endpoint, commitId, language, partial, final, token, data, accountToken, username, projectName) {
        return sendLanguage(endpoint, commitId, language, partial, final, token, data, accountToken, username, projectName)
            .then(function(err) {
                if (err) {
                    return Promise.reject(err);
                }
    ...
    function sendLanguage(endpoint, commitId, language, partial, final, token, data, accountToken, username, projectName) {
        var url;
        if (final) {
            url = endpoint.replace(':commitId', commitId).replace(':language', 'coverageFinal').replace('coverage', 'commit');
            logger.debug('Reporting that all reports were sent');
        } else {
            url = endpoint.replace(':commitId', commitId).replace(':language', language);
        }
        // jscs:disable
        ...
        if (partial) {
            url += '?partial=true';
            logger.debug('Sending partial coverage');
        }
    ...
    module.exports = function(options) {
        ...
        return {
            sendCoverage: function sendCoverage(token, commitId, language, partial, final, data, accountToken, username, projectName) {
                ...
                if (language) {
                    return sendForLanguage(endpoint, commitId, language, partial, final, token, data, accountToken, username, projectName);
                }

                return sendByLanguage(endpoint, commitId, partial, final, token, data, accountToken, username, projectName);
            }
        };
    };
